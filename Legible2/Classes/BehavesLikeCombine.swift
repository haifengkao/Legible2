...
public extension Publisher {
                       ...
                       line: UInt = #line) -> CombineExpectation where Output == Void
    {
        expecting { expectation in
            sink(receiveCompletion: shouldNotBeCalled(_:)) { value in
                expect(file: file, line: line, value) == ()
                expectation.fulfill()
            }
        }
    }
...
public extension WhenPublisher where P: Subject {
    func sends(_ value: P.Output) -> CombineAction {
        expectation.when {
            publisher.send(value)
        }
    }

    func fails(with error: P.Failure) -> CombineAction {
        expectation.when {
...
internal class BehavesLikeCombineChain: Behavior<CombineAction> {
    override public class func spec(_ aContext: @escaping () -> CombineAction) {
        context("stream of events") {
            ...
            it("should fullfil expectations") {
                inContext.action?()
                if XCTWaiter().wait(for: [expectation],
                                    timeout: inContext.waiting.timeout) != .completed
                {
...
public class BehavesLikeCombine<T>: Behavior<T> {
    override public class func spec(_ aContext: @escaping () -> T) {
